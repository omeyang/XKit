...
// newAsyncCancelCtx 返回供异步检查使用的可取消 context。
// 拆出独立函数：cancel 跨 New→doShutdown 生命周期，
// 无法在 New 内 defer，包裹后避开 gosec G118 的同函数 defer 检查。
func newAsyncCancelCtx() (context.Context, context.CancelFunc) {
...
func New(opts ...Option) (*Health, error) {
	...
	asyncCtx, asyncCancel := newAsyncCancelCtx()
	h := &Health{
		opts:        o,
...
func (h *Health) addCheck(ep endpoint, name string, cfg CheckConfig) error {
	...
	if cfg.Async && h.started.Load() {
		return fmt.Errorf("%w: cannot add async check after Run", ErrAlreadyStarted)
	}
...
func (h *Health) Run(ctx context.Context) error {
	...
	if err != nil {
		...
		return fmt.Errorf("%w: %w", ErrInvalidAddr, err)
	}
	...
	// 设计决策: 补偿关闭。如果在 Listen 和 server.Store 之间已调用 Shutdown，
	// stopOnce 已执行但 server 尚未 Store（此时 Load 返回 nil），导致 Serve 永久运行。
	// 此处检查 shutdown 标志并补偿关闭 server，使 Serve 立即返回 ErrServerClosed。
	if h.shutdown.Load() {
		if err := srv.Close(); err != nil {
			slog.Warn("xhealth: compensated close after early shutdown", "err", err)
		}
	}
	...
	err = srv.Serve(ln)
	// 确保异步 goroutine 收到停止信号（Serve 可能因非 ErrServerClosed 错误返回）。
	// doShutdown 幂等，由 stopOnce 保护。
	h.doShutdown()
	wg.Wait()
...
func (h *Health) doShutdown() {
	h.stopOnce.Do(func() {
		...
		// 通知所有端点状态变更为 Down。
		// 在 close(stopCh) 之前通知，确保回调在异步 goroutine 停止前执行。
		for i := range h.statuses {
			h.maybeNotifyStatusChange(endpoint(i), StatusDown)
		}
...
func (h *Health) executeCheck(ctx context.Context, entry *checkEntry) CheckResult {
	...
	if h.opts.cacheTTL > 0 {
		...
		// 设计决策: 使用 context.Background() 而非请求 ctx,避免单个请求取消后
		// 将 context.Canceled 结果写入缓存,污染后续探针。
		// 闭包始终返回 nil error；若未来修改返回错误则记录以便定位。
		v, err, _ := entry.sf.Do("", func() (any, error) {
			if cached, ok := entry.getCached(); ok {
				return cached, nil
			}
			r := entry.execute(context.Background())
...
func (h *Health) startAsyncChecks(wg *sync.WaitGroup) {
	...
	for epIdx := range h.checks {
		for _, entry := range h.checks[epIdx] {
			...
			wg.Add(1)
			go h.runAsyncCheck(wg, endpoint(epIdx), entry)
		}
	}
}

// runAsyncCheck 后台定期执行异步检查。
func (h *Health) runAsyncCheck(wg *sync.WaitGroup, ep endpoint, entry *checkEntry) {
	defer wg.Done()

	result := entry.execute(h.asyncCtx)
	entry.setCached(result)
	h.maybeNotifyStatusChange(ep, h.aggregateEndpointStatus(ep))
...
// aggregateEndpointStatus 从缓存结果聚合端点状态（不执行检查）。
func (h *Health) aggregateEndpointStatus(ep endpoint) Status {
	h.mu.RLock()
	entries := h.checks[ep]
	h.mu.RUnlock()
	...
	status := StatusUp
	for _, entry := range entries {
		cr, ok := entry.getCached()
		if !ok {
			continue
		}
		switch {
		case cr.Status == StatusDown && status != StatusDown:
			status = StatusDown
		case cr.Status == StatusDegraded && status == StatusUp:
...
func (h *Health) maybeNotifyStatusChange(ep endpoint, newStatus Status) {
	...
	h.safeCallStatusListener(endpointNames[ep], oldStatus, newStatus)
}

// safeCallStatusListener 安全调用用户提供的状态监听回调。
//
// 内部 recover 用户回调的 panic，避免异步 goroutine 或 doShutdown 路径中的
// panic 杀死进程（这些路径不受 net/http 的 recover 保护）。
func (h *Health) safeCallStatusListener(epName string, oldStatus, newStatus Status) {
	defer func() {
		if r := recover(); r != nil {
			slog.Error("xhealth: status listener panic recovered",
				"endpoint", epName, "old_status", string(oldStatus),
				"new_status", string(newStatus), "panic", r)
		}
	}()
	h.opts.statusListener(epName, oldStatus, newStatus)
}

...
// =============================================================================
// Additional Backoff Tests
// =============================================================================

func TestBackoffConfig_DelayCalculation(t *testing.T) {
	// 测试退避延迟计算逻辑
	config := BackoffConfig{
	...
	// 第一次退避
	assert.Equal(t, 100*time.Millisecond, currentDelay)
	...
	assert.Equal(t, 200*time.Millisecond, currentDelay)
	...
	currentDelay = nextDelay
	assert.Equal(t, 400*time.Millisecond, currentDelay)
	...
	currentDelay = nextDelay
	assert.Equal(t, 800*time.Millisecond, currentDelay)
	...
	currentDelay = nextDelay
	assert.Equal(t, 1*time.Second, currentDelay) // 被限制在 MaxDelay
}

func TestBackoffConfig_LargeMultiplier(t *testing.T) {
	config := BackoffConfig{
		InitialDelay: 100 * time.Millisecond,
		MaxDelay:     1 * time.Second,
		Multiplier:   10.0,
	...
	// 应该直接达到最大值
	assert.Equal(t, 1*time.Second, nextDelay)
}

func TestBackoffConfig_SmallMultiplier(t *testing.T) {
	config := BackoffConfig{
		InitialDelay: 100 * time.Millisecond,
		MaxDelay:     1 * time.Second,
		Multiplier:   1.1,
	...
	// 小乘数应该缓慢增长
	assert.Equal(t, 110*time.Millisecond, nextDelay)
}

	...
	for {
		...
		case resp, ok := <-watchCh:
			...
			if resp.Err() != nil {
				...
				sendErrorEvent(ctx, eventCh, resp.Err(), lastRevision, resp.CompactRevision)
				return
			}
...
// sendErrorEvent 发送错误事件到通道（纯函数，不依赖客户端状态）。
// lastRevision 是最后成功处理的 revision，便于调用方使用 WithRevision 恢复 Watch。
// compactRevision 是 etcd 的压缩版本号，当错误由 compaction 引起时非零。
func sendErrorEvent(ctx context.Context, eventCh chan<- Event, err error, lastRevision, compactRevision int64) {
	select {
...
// disconnectErrOrDefault 返回断开错误，无错误时使用通用描述。

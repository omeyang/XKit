...
// # URL 编码注意事项
//
// 本包处理文件系统路径，不处理 URL 编码。如 "%2e%2e" 被视为合法的文件名字符，
// 不会被识别为路径穿越。如果输入来自 HTTP 请求，调用方必须先完成 URL 解码再传入。
//
// # 符号链接安全注意事项
//
...
//
// 注意：符号链接检查存在 TOCTOU 窗口（检查与使用之间符号链接可能被修改）。
// 如需更强保证，应配合操作系统权限控制（如禁止在 base 内创建符号链接）。
//
// # 错误处理
//

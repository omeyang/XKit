...
# 向 .adversarial-runs/review-log.md 追加失败条目（仅在真正失败路径调用）
append_failure_log() {
...
# === 固定 SLOT→包 映射（2026-05-15 用户指定，长期生效）===
# 调度：cron 每天 0-14 点整点各跑一次（共 15 个 slot），slot=SLOT 直接对应 PKGS[SLOT]。
# 选包标准：聚焦"历史审查轮次较少"的包——xauth/xdlock/xhealth 已 8+ 轮无新真问题，
#         从自动化清单中移除，需要时手动 `./scripts/adversarial-review.sh <slot>` 调用。
# 顺序：欠审包在前（slot 0..4，2-3 轮），新加入的 4 轮包在后（slot 5..14）——万一某天
#       cron 早段失败，损失的是回归扫描而非补审，欠审包优先保住。
# 变更门槛：用户长期固定意图。未经用户明确许可不要扩/缩此清单或调换顺序。
# 路径覆盖：PKG_DIR find -maxdepth 3 覆盖 pkg/**、cmd/*、internal/*（含
#         pkg/distributed/xsemaphore/xsemaphoremock：从 pkg/ 起算 depth=3，正好命中）。
PKGS=(
  rediscompat                           # slot=0   internal/*，2 轮（欠审）
  xsemaphoremock                        # slot=1   pkg/distributed/xsemaphore/xsemaphoremock，2 轮（欠审）
  storageopt                            # slot=2   internal/*，3 轮（欠审）
  mqcore                                # slot=3   internal/*，3 轮（欠审）
  deploy                                # slot=4   internal/*，3 轮（欠审）
  xtrace                                # slot=5   pkg/observability/xtrace，4 轮
  xctx                                  # slot=6   pkg/context/xctx，4 轮
  xutil                                 # slot=7   pkg/util/xutil，4 轮
  xdbgctl                               # slot=8   cmd/xdbgctl，4 轮
  xrotate                               # slot=9   pkg/observability/xrotate，4 轮
  xmac                                  # slot=10  pkg/util/xmac，4 轮
  xlru                                  # slot=11  pkg/util/xlru，4 轮
  xkafka                                # slot=12  pkg/mq/xkafka，4 轮
  xsemaphore                            # slot=13  pkg/distributed/xsemaphore，4 轮
  xsampling                             # slot=14  pkg/observability/xsampling，4 轮
)
N=${#PKGS[@]}
if (( SLOT >= N )); then
  echo "SLOT=$SLOT >= N=$N，超出固定映射范围，跳过（应同步删 crontab 多余调度）"
  exit 0
fi
TARGET="${PKGS[$SLOT]}"
echo "fixed map: SLOT=$SLOT -> TARGET=$TARGET (N=$N)"

PKG_DIR=$(find "$REPO/pkg" "$REPO/cmd" "$REPO/internal" -maxdepth 3 -type d -name "$TARGET" 2>/dev/null | head -1)
if [[ -z "$PKG_DIR" ]]; then
  echo "TARGET not found under pkg/**, cmd/* or internal/*; skip"
  {
    [[ -f "$LOG_FILE" ]] || echo "# Adversarial Review Log"
    echo ""
    echo "## $(TZ=Asia/Shanghai date +%Y-%m-%d) slot=$SLOT TARGET=$TARGET"
    echo "- 状态: 包不存在于 pkg/**、cmd/* 或 internal/* 下，跳过"
  } >> "$LOG_FILE" 2>/dev/null || true
  exit 0
fi
